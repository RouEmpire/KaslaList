...
describe("Type class and it's object behavior", () => {  
  ...
  test("Insert query correctness with description", () => {
    // setup
    const today = new Date();
    const testTodoItem = new TodoItem("Water plants", today, "water the plants near windows")
    const expectResult:string = `('Water plants', ${today.getTime()}, 0, -1, 'water the plants near windows')`
    // verify
    expect(testTodoItem.SQLiteInsert).toBe(expectResult);
  })
  ...
  test("as object getter (.itemAsObject)", () => {
    // setup
    const today = new Date();
    const testTodoItem = new TodoItem("Water plants", today, "water the plants near window")
    const expectResult = {
      title: "Water plants",
    ...
    // verify
    expect(testTodoItem.itemAsObject).toMatchObject(expectResult);
  })
});
